...
const CustomDropdown = ({
  ...
  const [dropdowninputfield, setdropdowninputfield] = useState('Add');
  ...
  useEffect(() => {
    if (dropdownLabel) {
      setdropdowninputfield(dropdownLabel);
    }
  }, [dropdownLabel]);
  ...
  return (
    <>
      <TouchableOpacity style={styles.dropdownBox} onPress={() => setDropdownOpen(true)}>
        <Text style={styles.dropdownTextInput}>
          {selectedOption && !selectedOption[_optionLabel] ? (
            <Text style={{ color: '#d4d2d2' }}>{`${dropdowninputfield}`}</Text>
          ) : (
            selectedOption[_optionLabel]
          )}
        </Text>
      ...
      <View>
        <Portal>
          <Dialog visible={dropdownOpen} style={styles.dialogContent} onDismiss={() => setDropdownOpen(false)}>
                  ...
                  >
                    {_dropdownType === 'single' ? (
                      ...
                      <Text
                        key={option.id}
                        style={
                          selectedMultiOption.length > 0 && selectedMultiOption.includes(option)
                            ? styles.selectedMulOptionText
                            : styles.optionText
                        }
                      >
                        {option[_optionLabel]}
            ...
            {displayButton ? (
              <View>
                <TouchableOpacity onPress={() => handleAddButton()} style={styles.addBtn}>
            ...
            {selectedMultiOption.map((option, index) => (
              <View style={styles.multiOptPot}>
                ...
                <TouchableOpacity onPress={() => unSelect(option)}>
                  <MaterialCommunityIcons name='delete' color={'#fff'} size={18} style={{ marginLeft: 10 }} />
                </TouchableOpacity>
              </View>
            ))}
          </View>
