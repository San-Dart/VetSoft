...
const AddMedicineToPrescription = ({ route, navigation }) => {
  ...
  const [coursesComponentElementData, setCoursesComponentElementData] =
    useState([]);
  ...
  useEffect(() => {
    if (route.params.editMedicineDetails) {
      ...
      setMedicineType(
        route.params.editMedicineDetails.medicine.medicine_categories
          .medicine_type
      );
  ...
  return (
    <ScrollView>
      ...
      {coursesComponent}
      <TouchableOpacity onPress={onDone} style={styles.done}>
        <Text
