...
const renderHeader = (item, _) => {
  ...
  return (
    <View style={styles.header} key={_}>
      <View style={styles.entire_box}>
        <View style={{ backgroundColor: '#fff', paddingLeft: 20, borderRadius: 20 }}>
          <MaterialCommunityIcons name={item.actual_animal_type} color={'grey'} size={40} />
        </View>
    ...
    <>
      <View style={{ marginBottom: 80 }}>
        {petData.map((item, index) => (
          <ScrollView showsVerticalScrollIndicator={false} key={'pet_' + index}>
            <View>
              <>
                <Accordion
...
//   const handleSubmit = () => {
//     axios
//       .put(`/petOwner/update/${ownerId}`, formData)
//       .then((res) => {
//         if (res.status == '200') {
...
//             onChangeText={(value) => {
//               value && handlePetOwnerNameChange(value);
//             }}
...
//             style={styles.textInp}
//             onChangeText={(value) => {
//               value && handleContactNumberChange(value);
//             }}
//           />
//         </View>
...
//             style={styles.textInp}
//             onChangeText={(value) => {
//               value && handleAddressChange(value);
//             }}
//           />
//         </View>
...
//         >
//           <Text style={{ color: '#fff', textAlign: 'center', fontSize: 16, fontWeight: 'bold' }}>Done</Text>
//         </TouchableOpacity>
//         <View>
//           <>
//             {successMsg ? (
...
const PetOwnerDetail = ({ route, navigation }) => {
  ...
  navigation.setOptions({ title: `Pet Owners` });
  ...
  const [petsNav, setPetsNav] = useState(true);
  ...
  const petsOfOwnerDetail = async () => {
    await axios
      .get(`/pet/owner/${sel_pet_owner_id}`)
      .then((res) => {
        if (res.status === 200) {
          // setPetData(petData);
  ...
  const func_owner = (details) => {
    details.map((item, index) => {
      setOwner_name(item.pet_owner_id.pet_owner_name);
      setOwner_mobile(item.pet_owner_id.contact_number);
    });
  };
