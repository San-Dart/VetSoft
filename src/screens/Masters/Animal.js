...
const ListItem = ({ item, selected, onPress, onLongPress }) => (
  <>
    <TouchableOpacity onPress={onPress} onLongPress={onLongPress} style={styles.listItem}>
      <View style={{ justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', width: '100%' }}>
        <View style={{ backgroundColor: '#fff', borderRadius: 50, padding: 10, elevation: 2 }}>
          <MaterialCommunityIcons name='dog' color={'#006766'} size={30} />
        </View>
      ...
      .then((res) => {
        ...
        res.data.map((element, index) => {
          ...
          animalData.push({
            ...
            default_color_id: element.edited_default_color_id
              ? element.edited_default_color_id
              : element.actual_default_color_id,
            default_breed_id: element.edited_default_breed_id
              ? element.edited_default_breed_id
              : element.actual_default_breed_id,
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        <View>
          <FAB style={styles.fab} medium icon='plus' color='#fff' onPress={() => navigation.navigate('AddAnimal')} />
        </View>
