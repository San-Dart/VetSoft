...
const Visits = ({ route, navigation, selected }) => {
  ...
  const onChangeFromDob = (event, selectedDate) => {
  ...
  const onChangeToDob = (event, selectedDate) => {
    const currentDate = selectedDate || date;
    setToShow(Platform.OS === 'ios');
    setToDate(currentDate);

    let tempDate = new Date(currentDate);
    let tDate = tempDate.getDate() + '/' + (tempDate.getMonth() + 1) + '/' + tempDate.getFullYear();
    setToText(tDate);
  };
  ...
  return (
    <>
        ...
        <View style={{ flexDirection: 'row', marginTop: 20 }}>
          <View style={{ width: '50%' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>
              <MaterialIcons name='calendar-today' color={'#d4d2d2'} size={30} />
              <View
                style={{
                  width: '80%',
                  borderRadius: 15,
              ...
              >
                <TouchableOpacity onPress={() => showFromMode('date')}>
                  <View>
                    {fromtext ? (
                      <Text style={{ color: '#000' }}>{fromtext}</Text>
                    ) : (
          ...
          <View style={{ width: '50%' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>
              ...
              >
                <TouchableOpacity onPress={() => showToMode('date')}>
                  <View>
                    {totext ? (
                      <Text style={{ color: '#000' }}>{totext}</Text>
                    ) : (
                      <Text style={{ color: '#d4d2d2' }}>Select date</Text>
                    )}
                  </View>
            ...
            <Pressable onPress={deSelectItems} key={index}>
              ...
              >
                ...
                >
                  ...
                  >
                    <Text>
                      Pet Name: <Text style={{ fontWeight: 'bold' }}>{element.pet_id.pet_name}</Text>
                    </Text>
                    <Text>
                      Owner Name: <Text style={{ fontWeight: 'bold' }}>{element.owner_name_id.pet_owner_name}</Text>
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  >
                    <Text>
                      Clinic: <Text style={{ fontWeight: 'bold' }}>{element.visited_clinic_id.clinic_name}</Text>
                    </Text>
                    <Text>
                      Doctor Name: <Text style={{ fontWeight: 'bold' }}>{element.doctor_name}</Text>
                    </Text>
                  </View>
                </View>
              </TouchableOpacity>
      ...
      <View style={styles.floatButtons}>
        <View>
          <FAB style={styles.fab} medium icon='plus' color='#fff' onPress={() => navigation.navigate('NewVisit')} />
        </View>
        <View>
